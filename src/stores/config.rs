...
impl ConfigStore {
    pub fn new(is_advanced_settings: bool) -> ConfigStore {
        ConfigStore { is_advanced_settings }
    }
    ...
    pub fn store(&self) -> Result<(), &str> {
        ...
        match fs::write(&path, serialized_store) {
            ...
            Err(err) => {
                error!("Failed on saving app config: {}", err);
                Err("Failed on saving app config")
            }
        }
    }
    // Load config from user directory
    pub fn from_config() -> Result<ConfigStore, ()> {
        ...
        match config_path {
            Some(path) => {
                ...
                let config: ConfigStore = match serde_json::from_str(&json) {
                    Ok(app_store) => app_store,
                    Err(err) => {
                        error!("Failed parsing config from JSON: {}", err);
                        return Err(());
                    }
                };
                Ok(config)
...
pub fn use_config_store() {
    trace!("Init config store...");


    use_context_provider(|| Signal::new(ConfigStore::from_config().unwrap_or(
        ConfigStore::new(
            false,
        )
    )));
